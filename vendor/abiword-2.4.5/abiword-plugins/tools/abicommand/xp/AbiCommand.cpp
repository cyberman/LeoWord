...
AbiCommand::AbiCommand (void) :
	...
	m_pG (NULL),
	m_pLayout (NULL),
	m_bViewDoc (false), m_bRunAsServer (false), m_iPID (0), m_sErrorFile ("")
{
...
AbiCommand::doCommands (void)
{
	...
	while (!bQuit)
	{
		...
		if (toks.getItemCount () > 0)
		{
			...
			if (pTok && UT_strcmp (pTok->utf8_str (), "quit") == 0
				|| UT_strcmp (pTok->utf8_str (), "q") == 0)
				bQuit = true;
			else
			{
...
AbiCommand::parseTokens (UT_Vector * pToks)
{
	...
	// New document
	//
	if (UT_strcmp (pCom0->utf8_str (), "new") == 0)
	{
	...
	// Load in a document
	//
	if (UT_strcmp (pCom0->utf8_str (), "load") == 0)
	{
	...
	// printfile
	//
	else if (UT_strcmp (pCom0->utf8_str (), "printfile") == 0)
	{
		if (count >= 2)
		{
			if (printFiles (pToks))
	...
	// inserttext
	//
	else if (UT_strcmp (pCom0->utf8_str (), "inserttext") == 0)
	{
		if (count >= 2)
		{
			if (insertText (pToks))
	...
	// delete
	//
	else if (UT_strcmp (pCom0->utf8_str (), "delete") == 0)
	{
		if (deleteText (pToks))
			return 0;
	...
	// Replace Next
	//
	else if (UT_strcmp (pCom0->utf8_str (), "replacenext") == 0)
	{
		if (count > 2)
	...
	// replaceAll
	//
	else if (UT_strcmp (pCom0->utf8_str (), "replaceall") == 0)
	{
		if (count > 2)
	...
	//
	else if (UT_strcmp (pCom0->utf8_str (), "movept") == 0)
	{
		if (count > 1)
	...
	//
	else if (UT_strcmp (pCom0->utf8_str (), "visualedit") == 0)
	{
	...
	// Start selection
	//
	else if (UT_strcmp (pCom0->utf8_str (), "selectstart") == 0)
	{
		if (m_pCurView)
		{
	...
	// Clear selection
	//
	else if (UT_strcmp (pCom0->utf8_str (), "selectclear") == 0)
	{
		if (m_pCurView)
		{
	...
	// findnext
	//
	else if (UT_strcmp (pCom0->utf8_str (), "findnext") == 0)
	{
	...
	// Save
	//
	else if (UT_strcmp (pCom0->utf8_str (), "save") == 0)
	{
		if (m_pCurDoc)
		{
			IEFileType ieft = 0;
			...
			if (pToks->getItemCount () > 1)
			{
				...
				m_pCurDoc->saveAs (pCom1->utf8_str (), ieft);

				return 0;
			}

			ieft = static_cast < IEFileType > (m_pCurDoc->getLastOpenedType ());
			m_pCurDoc->saveAs (m_pCurFile->utf8_str (), ieft);
	...
	// Convert to Text
	//
	else if (UT_strcmp (pCom0->utf8_str (), "converttotext") == 0 ||
			 UT_strcmp (pCom0->utf8_str (), "convert") == 0)
	{
	...
	// Write PID to file
	//
	else if (UT_strcmp (pCom0->utf8_str (), "writepid") == 0)
	{
		if (pToks->getItemCount () < 2)
			return -1;
	...
	// server
	//      
	else if (UT_strcmp (pCom0->utf8_str (), "server") == 0)
	{
		if (pToks->getItemCount () < 2)
			return -1;
	...
	// PNG preview
	//
	else if (UT_strcmp (pCom0->utf8_str (), "previewpng") == 0)
	{
	...
	// Help
	//
	else if (UT_strcmp (pCom0->utf8_str (), "help") == 0)
	{
		...
		printf ("replaceall <find> <target>  - Replace every occurance of <find> with <target>\n");
		printf ("                              in the current document.\n");
		printf ("replacenext <find> <target> - Replace the next occurance of <find> with <target>\n");
		printf ("                              in the current document.\n");
		...
		printf ("                              If filename is omitted the file is saved to it's\n");
		...
		printf ("                              third paramter (abw,html,odt, etc.).\n");
...
AbiCommand::movePoint (UT_Vector * pToks)
{
	if (m_pCurView != NULL)
	{
		...
		if (UT_stricmp (pTarget->utf8_str (), "BOD") == 0)
			docpos = FV_DOCPOS_BOD;
		else if (UT_stricmp (pTarget->utf8_str (), "EOD") == 0)
			docpos = FV_DOCPOS_EOD;
		else if (UT_stricmp (pTarget->utf8_str (), "BOB") == 0)
			docpos = FV_DOCPOS_BOB;
		else if (UT_stricmp (pTarget->utf8_str (), "EOB") == 0)
			docpos = FV_DOCPOS_EOB;
		else if (UT_stricmp (pTarget->utf8_str (), "BOP") == 0)
			docpos = FV_DOCPOS_BOP;
		else if (UT_stricmp (pTarget->utf8_str (), "EOP") == 0)
			docpos = FV_DOCPOS_EOP;
		else if (UT_stricmp (pTarget->utf8_str (), "BOL") == 0)
 			docpos = FV_DOCPOS_BOL;
		else if (UT_stricmp (pTarget->utf8_str (), "EOL") == 0)
			docpos = FV_DOCPOS_EOL;
		else if (UT_stricmp (pTarget->utf8_str (), "BOS") == 0)
			docpos = FV_DOCPOS_BOS;
		else if (UT_stricmp (pTarget->utf8_str (), "EOS") == 0)
			docpos = FV_DOCPOS_EOS;
		else if (UT_stricmp (pTarget->utf8_str (), "BOW") == 0)
...
AbiCommand::printFiles (UT_Vector * pToks)
{
	for (UT_uint32 i = 1; i < pToks->getItemCount (); i++)
	{
		...
		PS_GraphicsAllocInfo
			ai (pFile->utf8_str (), m_pCurFile->utf8_str (),
				m_pApp->getApplicationName (),
				static_cast < XAP_UnixApp * >(m_pApp)->getFontManager (), true, m_pApp);

		PS_Graphics *pG = (PS_Graphics *) m_pApp->newGraphics (ai);
		...
		if (pG->startPrint ())
		{
			...
			for (UT_uint32 k = 1; (k <= pDocLayout->countPages ()); k++)
			{
				...
				pG->startPage (pFile->utf8_str (), k, orient, iWidth,
							   iHeight);
...
AbiCommand::replaceDocument (PD_Document * pDoc)
{
	...
	XAP_UnixNullGraphicsAllocInfo ai (pUnixApp->getFontManager (), m_pApp);

...
const XML_Char * szAbiPluginSchemeName = ABI_PLUGIN_SCHEME_NAME ;
 ...
 */
DECLARE_ABI_PLUGIN_METHOD(editImage);
ABI_GRAYABLE_MENUITEM_PROTOTYPE(editImage);
Defun_EV_GetMenuItemComputedLabel_Fn(getEditImageMenuName);
DECLARE_ABI_PLUGIN_METHOD(saveAsBmp);
DECLARE_ABI_PLUGIN_METHOD(specify);
DECLARE_ABI_PLUGIN_METHOD(useBmp);
...
#define NUM_MENUITEMS NrElements(amo)
...
{
    "AbiPaint",											/* name */
    "Allows editting an embedded image via external image editing program.",	/* desc */
...
//
				...
				errorCode = IE_ImpGraphic::constructImporter(szTmpPng.c_str(), iegft, &pIEG);
				if(errorCode)
				{
				...
				errorCode = pIEG->importGraphic(szTmpPng.c_str(), &pFG);
				if(errorCode)
				{
					UT_ASSERT(UT_SHOULD_NOT_HAPPEN);

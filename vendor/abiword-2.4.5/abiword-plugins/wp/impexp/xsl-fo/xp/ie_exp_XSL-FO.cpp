...
public:
	...
	void setIdList(UT_uint32 id)
	{
		...
		if (it.is_valid())
		{
			m_pan = reinterpret_cast<fl_AutoNum* const> (static_cast<const UT_Pair<void*,void*>*> (it.value())->second());
			UT_ASSERT(m_pan);
...
#if 0
	  pSectionAP->getProperty("table-col-spacing", static_cast<const XML_Char *>(&pszTableColSpacing));
	  pSectionAP->getProperty("table-row-spacing", static_cast<const XML_Char *>(&pszTableRowSpacing));
	  pSectionAP->getProperty("cell-margin-left", static_cast<const XML_Char *>(&pszLeftOffset));
	  pSectionAP->getProperty("cell-margin-top", static_cast<const XML_Char *>(&pszTopOffset));
	  pSectionAP->getProperty("cell-margin-right", static_cast<const XML_Char *>(&pszRightOffset));
	  pSectionAP->getProperty("cell-margin-bottom", static_cast<const XML_Char *>(&pszBottomOffset));
#endif
...
void s_XSL_FO_Listener::_handleField(PT_AttrPropIndex api)
{
	...
	if (bHaveProp && pAP)
	{
		...
		{
			if (szValue[0] == 'l' && strcmp(static_cast<const char*>(szValue), "list_label") == 0)
			{
	...
	case PX_ChangeRecord::PXT_InsertObject:
		{
			...
			case PTO_Image:
			{
			  ...
			  if(bHaveProp && pAP && pAP->getAttribute("dataid", szValue))
			    {
			      const char* dataid = strdup(static_cast<const char*>(szValue));
...
#define PROPERTY(x) \
	if (pAP->getProperty(x, szValue)) \
		content_st << " "x"=\"" << static_cast<const char*>(szValue) << "\"";
...
void s_XSL_FO_Listener::_openBlock(PT_AttrPropIndex api)
{
	...
	if (bHaveProp && pAP)
	{
		if (pAP->getProperty("bgcolor", szValue))
		{
			...
			content_st << static_cast<const char*>(szValue) << "\"";
		}
		...
		if (pAP->getProperty("lang", szValue))
			content_st << " language=\"" << static_cast<const char*>(szValue) << "\"";

		if (pAP->getProperty("font-size", szValue))
			content_st << " font-size=\"" << purgeSpaces(static_cast<const char *>(szValue)).c_str() << "\"";
	...
	if (pAP && pAP->getAttribute("listid", szValue))
	{
		...
		UT_uint32 id = static_cast<UT_uint32>(atoi(static_cast<const char*>(szValue)));
		m_List.setIdList(id);
	}
...
void s_XSL_FO_Listener::_openSpan(PT_AttrPropIndex api)
{
	...
	if (bHaveProp && pAP)
	{
		...
		if (pAP->getProperty("font-size", szValue))
		{
			...
			content_st += purgeSpaces(static_cast<const char *>(szValue)).c_str();
			content_st += "\"";
		}		

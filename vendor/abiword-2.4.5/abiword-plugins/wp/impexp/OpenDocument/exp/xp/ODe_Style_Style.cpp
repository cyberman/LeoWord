...
bool ODe_Style_Style::write(GsfOutput* pODT, const UT_UTF8String& rSpacesOffset) const {
    ...
    ODe_writeAttribute(output, "style:parent-style-name", m_parentStyleName);
    ODe_writeAttribute(output, "style:next-style-name", m_nextStyleName);
    ODe_writeAttribute(output, "style:master-page-name", m_masterPageName);
...
bool ODe_Style_Style::fetchAttributesFromAbiStyle(const PP_AttrProp* pAP) {
    ...
    if (ok && pValue != NULL) {
        ...
        if (UT_strcmp("Current Settings", pValue) != 0) {
 ...
 * 
 */
void ODe_Style_Style::setBreakBefore(const XML_Char* pBreakBefore) {
    if (m_pParagraphProps == NULL) {
        m_pParagraphProps = new ParagraphProps();
    }
 ...
 * 
 */
void ODe_Style_Style::setColumnWidth(const XML_Char* pColumnWidth) {
 ...
 * 
 */
void ODe_Style_Style::setRowHeight(const XML_Char* pRowHeight) {
    if (m_pRowProps == NULL) {
...
write(UT_UTF8String& rOutput, const UT_UTF8String& rSpacesOffset) const {
    ...
    // Bugged, must realize how to translate it correctly form AbiWord to
    // OpenDocument
...
fetchAttributesFromAbiProps(const PP_AttrProp& rAP) {
    ...
    if (ok && pValue != NULL) {
        if (UT_XML_strlen(pValue)==6) {
            ...
            UT_ASSERT(UT_XML_strlen(pValue)==7);
            UT_ASSERT(pValue[0]=='#');
            m_color = pValue;
        }
    }
    ...
    if (ok && pValue != NULL) {
        ...
        while (keepGoing) {
            ...
            if (finishedWord) {
                if (!UT_strcmp(buffer, "underline")) {
                    m_underlineType = "single";
                    
                } else if (!UT_strcmp(buffer, "line-through")) {
    ...
    if (ok && pValue != NULL) {
        if (!UT_strcmp("subscript", pValue)) {
            // Hard coded, it's ugly but I can't do otherwise.
            m_textPosition = "-33%";
        } else if (!UT_strcmp("superscript", pValue)) {
            // Hard coded, it's ugly but I can't do otherwise.
            m_textPosition = "33%";
        } else {
            UT_ASSERT( !UT_strcmp("normal", pValue) );
    ...
    if (ok && pValue != NULL) {
        if (!UT_strcmp(pValue, "-none-")) {
            m_language = "none";
            m_country = "none";
        } else {
            XML_Char strLanguage[3];
            XML_Char strCountry[3];

            // All valid strings should have 5 chars. e.g.: "en-US"
            UT_ASSERT( UT_XML_strlen(pValue) == 5); 
            
            if (UT_XML_strlen(pValue) == 5) {
...
fetchAttributesFromAbiProps(const PP_AttrProp& rAP) {
    ...
    if (ok && pValue != NULL) {
        if (!UT_strcmp(pValue, "above-text")) {
            m_wrap = "run-through";
            m_runThrough = "foreground";
        } else if (!UT_strcmp(pValue, "wrapped-both")) {
            m_wrap = "parallel";
        } else {
            UT_ASSERT(UT_SHOULD_NOT_HAPPEN);
        }
    ...
    if (!UT_strcmp(pValue, "block-above-text")) {

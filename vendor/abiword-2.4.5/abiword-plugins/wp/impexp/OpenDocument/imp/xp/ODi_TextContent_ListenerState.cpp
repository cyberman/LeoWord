...
ODi_TextContent_ListenerState::~ODi_TextContent_ListenerState() {
    ...
    UT_ASSERT(m_tablesOfContentProps.getItemCount()==0);
 ...
 */
void ODi_TextContent_ListenerState::startElement (const XML_Char* pName,
                                          const XML_Char** ppAtts,
                                          ODi_ListenerStateAction& rAction)
{
    if (UT_strcmp(pName, "text:section" ) != 0 ) {
    ...
    if (!UT_strcmp(pName, "text:section" )) {
    ...
    } else if (!UT_strcmp(pName, "text:list")) {
    ...
    } else if (!UT_strcmp(pName, "text:list-item")) {
        m_alreadyDefinedAbiParagraphForList = false;
        
    } else if (!UT_strcmp(pName, "draw:frame")) {
        
        if (!UT_strcmp(m_rElementStack.getStartTag(0)->getName(), "text:p") ||
            !UT_strcmp(m_rElementStack.getStartTag(0)->getName(), "text:h")) {
        ...
        } else if (!UT_strcmp(m_rElementStack.getStartTag(0)->getName(), "office:text")) {
        ...
        } else if (!UT_strcmp(m_rElementStack.getStartTag(0)->getName(),
                              "text:span")) {
            ...
            if (!UT_strcmp(pVal, "as-char") || !UT_strcmp(pVal, "char")) {
                _flush();
                rAction.pushState("Frame");
            } else {
                // Ignore this element
    ...
    }  else if (!UT_strcmp(pName, "text:note")) {

        _flush();
        m_bAcceptingText = false;
        
    } else if (!UT_strcmp(pName, "text:note-body")) {

        const XML_Char* ppAtts[10];
        bool ok;
        UT_uint32 id;
        ...
        UT_uint8 i;
        bool isFootnote;
        const XML_Char* pNoteClass;
        
        pNoteClass = m_rElementStack.getStartTag(0)->getAttributeValue("text:note-class");
        UT_ASSERT(pNoteClass != NULL);
        
        if (!UT_strcmp(pNoteClass, "footnote")) {
            isFootnote = true;
        } else if (!UT_strcmp(pNoteClass, "endnote")) {
            isFootnote = false;
        } else {
            UT_ASSERT(UT_SHOULD_NOT_HAPPEN);
 ...
 * @param pName The name of the element
 */
void ODi_TextContent_ListenerState::endElement (const XML_Char* pName,
                                               ODi_ListenerStateAction& rAction)
{
    UT_ASSERT(m_elementParsingLevel >= 0);
 ...
 * 
 */
void ODi_TextContent_ListenerState::_insertBookmark (const XML_Char* pName,
 ...
 * 
 */
bool ODi_TextContent_ListenerState::_pushInlineFmt(const XML_Char ** atts)
{
 ...
 * 
 */
void ODi_TextContent_ListenerState::_insureInBlock(const XML_Char ** atts)
{
    if (m_bAcceptingText)
        return;
 ...
 */
void ODi_TextContent_ListenerState::_startParagraphElement (const XML_Char* pName,
                                          const XML_Char** ppParagraphAtts,
                                          ODi_ListenerStateAction& rAction) {
        bool bIsListParagraph = false;
        const XML_Char* pStyleName;
        const XML_Char *ppAtts[50];
        UT_uint8 i;
        XML_Char listLevel[10];
        bool ok;
        UT_UTF8String props;
        const ODi_Style_Style* pStyle;
        
        
        if (!UT_strcmp(m_rElementStack.getStartTag(0)->getName(), "text:list-item")) {
        ...
        if (bIsListParagraph && !m_alreadyDefinedAbiParagraphForList) {
            ...
            ppAtts[i] = 0; // Marks the end of the attributes list.
            m_pAbiDocument->appendStrux(PTX_Block, (const XML_Char**)ppAtts);
        }

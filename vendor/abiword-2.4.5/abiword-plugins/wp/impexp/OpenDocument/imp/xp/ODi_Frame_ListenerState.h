...
private:
     ...
     */
    void _drawImage (const XML_Char** ppAtts, ODi_ListenerStateAction& rAction);
    
    /**
     * @param ppAtts The attributes of a <draw:text-box> element.
     * @param rAction Any action to be taken, regarding state change.
     */
    void _drawTextBox (const XML_Char** ppAtts, ODi_ListenerStateAction& rAction);
    
    bool _getFrameProperties(UT_UTF8String& rProps, const XML_Char** ppAtts);

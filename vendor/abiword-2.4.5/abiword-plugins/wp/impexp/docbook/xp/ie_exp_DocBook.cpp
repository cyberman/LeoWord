...
{
	return (!UT_stricmp(szSuffix,".dbk") ||!UT_stricmp(szSuffix,".xml"));
}
...
void s_DocBook_Listener :: _closeSpan(void)
{
	...
	if (pAP)
	{
		...
		if (pAP->getProperty(static_cast<const XML_Char *>("text-position"), szValue))
		{
		...
		if ((pAP->getProperty(static_cast<const XML_Char *>("font-style"), szValue)) && !UT_strcmp(szValue, "italic"))
		{
...
void s_DocBook_Listener :: _openParagraph(PT_AttrPropIndex api)
{
	...
	if (bHaveProp && pAP)
	{
		...
		if (pAP->getAttribute(static_cast<const XML_Char *>(PT_STYLE_ATTRIBUTE_NAME), szValue))
		{	
			if((!UT_strcmp(szValue, "Heading 1")) || (!UT_strcmp(szValue, "Numbered Heading 1")))
			{
				// <p style="Heading 1"> ...
				_closeChapterTitle();
			...
			else if((!UT_strcmp(szValue, "Heading 2")) || (!UT_strcmp(szValue, "Numbered Heading 2")))
			{
				// <p style="Heading 2"> ...
				_closeChapterTitle();
				...
				if((!_inSectionStrux()) && !m_bInTitle)
				{
					_openSection(api, !UT_strcmp(szValue, m_sParentStyle.utf8_str()) ? m_iSectionDepth : 2, szValue);
			...
			else if((!UT_strcmp(szValue, "Heading 3")) || (!UT_strcmp(szValue, "Numbered Heading 3")))
			{
				// <p style="Heading 3"> ...
				_closeChapterTitle();
				...
				if(!_inSectionStrux() && !m_bInTitle)
				{
					_openSection(api, !UT_strcmp(szValue, m_sParentStyle.utf8_str()) ? m_iSectionDepth : 3, szValue);
					m_sParentStyle = szValue;
				}
				else
				{
			...
			else if(!UT_strcmp(szValue, "Heading 4")) 
			{
				// <p style="Heading 4"> ...
				_closeChapterTitle();
				...
				if(!_inSectionStrux() && !m_bInTitle)
				{
					_openSection(api, !UT_strcmp(szValue, m_sParentStyle.utf8_str()) ? m_iSectionDepth : 4, szValue);
					m_sParentStyle = szValue;
				}
				else
				{
			...
			else if (!UT_strcmp (szValue, "Chapter Heading"))
			{
				if(!_inSectionStrux() && !m_bInTitle)
				{
					_openChapter(api);
			...
			else if (!UT_strcmp (szValue, "Section Heading"))
			{
				_closeChapterTitle();
			...
			else if (!UT_strcmp (szValue, "Plain Text"))
			{
				_closeChapterTitle();
			...
			else if (!UT_strcmp (szValue, "Normal"))
			{
				_closeChapterTitle();
			...
			else
			{
				...
				if(!_inSectionStrux() && !m_bInTitle)
				{
					...
					if(UT_strcmp (szValue, m_sLastStyle.utf8_str()))  // not a coalescing run
					{
						_openSection(api,m_iSectionDepth,szValue);
						_closeSectionTitle(); //no title
					}
...
void s_DocBook_Listener :: _openSection (PT_AttrPropIndex api, int sub, const UT_UTF8String & content)
{
	...
	if(pAP && bHaveProp && (!UT_strcmp(content.utf8_str(),"abi-frame")))  //we only need this for frames
	{
		condition = _getProps(api);
		if(condition.length())
		{
	...
	{
		if(!UT_strcmp("",szValue))  // occurs with word documents
			return;
...
{
	if ((!m_bInParagraph && !m_bInTitle) || (m_iNestedTable == 1))
		return;
	...
	if (bHaveProp && pAP)
	{
		...
		if ((pAP->getAttribute(static_cast<const XML_Char *>("revision"), szValue)))
		{
			buf += " revision=\"";
			buf += szValue;
			buf += "\"";
		}
		if ((pAP->getProperty(static_cast<const XML_Char *>("lang"), szValue)))
		{
			buf += " lang=\"";
			buf += szValue;
			buf += "\"";
		}
		if ((pAP->getProperty(static_cast<const XML_Char *>("font-weight"), szValue)) && !UT_strcmp(szValue, "bold"))
		{
			buf += " role=\"strong\"";
		}
...
void s_DocBook_Listener::_handleImage(PT_AttrPropIndex api)
{
	...
	if(bHaveProp && pAP && pAP->getAttribute("dataid", szValue))
	{
		...
		if(pAP->getAttribute("title", szValue))  //use the image's title instead of it file name, if it exists
		{
			escaped = szValue;
			escaped.escapeXML();
			m_pie->write(escaped.utf8_str());
		...
		escaped += UT_basename(m_pie->getFileName());
		escaped += "_data/";
		escaped += buf.escapeXML();
		escaped += "\" format=\"PNG\"";
...
void s_DocBook_Listener::_handleField(const PX_ChangeRecord_Object * pcro, PT_AttrPropIndex api)
{
	...
	const PP_AttrProp * pAP = NULL;
	bool bHaveProp = m_pDocument->getAttrProp(api,&pAP);
	const XML_Char * szValue = 0, * szStyle = 0;
	...
	{
		if (!UT_strcmp (szValue, "list_label"))
		{
			_openList(api);
			return;
		}
		...
		if (!UT_strcmp (szValue, "endnote_anchor"))  //give the endnote <para> a unique id
		{
		...
		if (!UT_strcmp (szValue, "footnote_ref"))
		{
		...
		else if (!UT_strcmp (szValue, "endnote_ref"))
		{
...
void s_DocBook_Listener::_handleBookmark(PT_AttrPropIndex api)
{
	...
	if(bHaveProp && pAP && pAP->getAttribute("type", szValue))
	{
		if (!UT_strcmp (szValue, "start") && pAP->getAttribute("name", szValue))
		{
			buf = "anchor id=\"";
			escaped = szValue;
			escaped.escapeXML();
...
const UT_UTF8String s_DocBook_Listener::_getProps(PT_AttrPropIndex api)
{
	...
	const PP_AttrProp * pAP = NULL;
	bool bHaveProp = m_pDocument->getAttrProp (api, &pAP);
	const XML_Char * szName = 0, * szValue = 0;
...
void s_DocBook_Listener::_handleDataItems(void)
{
	...
	for (UT_uint32 k=0; (m_pDocument->enumDataItems(k,NULL,&szName,&pByteBuf,reinterpret_cast<const void **>(&szMimeType))); k++)
	{
		...
		{
			if(UT_strcmp(const_cast<char*>(reinterpret_cast<const char*>(m_utvDataIDs[i])), szName) == 0)
			{
				loc = i;
				break;
			}

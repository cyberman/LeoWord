...
public:
  ...
  * to Abi's.
  */
  void appendPageMaster(const XML_Char * name, const XML_Char ** atts) 
  {
  ...
  */
  const XML_Char ** getAbiPageAtts(const XML_Char * masterName)
  {
    UT_return_val_if_fail(masterName != NULL, (const XML_Char **)m_pageAtts);

    if (UT_strcmp (m_name.c_str(), masterName))
...
private: 
  ...
  void parse (const XML_Char ** props)
  {
...
public:
  ...
  OO_Style (const XML_Char ** props, const PD_Style * pParentStyle, const bool bOpenDocument)
    : m_bColBreakBefore (false), m_bPageBreakBefore (false), m_pParentStyle (pParentStyle), m_bOpenDocument (bOpenDocument)
  {
    parse (props);
  }

  OO_Style (const OO_Style * other, const XML_Char ** props, PD_Style * pParentStyle, const bool bOpenDocument)
    : m_bColBreakBefore (false), m_bPageBreakBefore (false), m_pParentStyle (pParentStyle), m_bOpenDocument (bOpenDocument)
  {
    if (other)
      *this = *other;
  ...
  void parse (const XML_Char ** props)
  {
	...
	if (m_bOpenDocument) {
		undr = const_cast<const XML_Char *>(UT_getAttribute("style:text-underline-style", props));
		strk = const_cast<const XML_Char *>(UT_getAttribute("style:text-line-through-style", props));
	}
	else {
		undr = const_cast<const XML_Char *>(UT_getAttribute("style:text-underline", props));
		strk = const_cast<const XML_Char *>(UT_getAttribute("style:text-crossing-out", props));
	}
	if (undr || strk) {
    ...
    if (val)
      m_keepWithNext = UT_String_sprintf ("keep-with-next: %s;", !UT_strcmp(val, "true") ? "yes" : "no");
    
    val = UT_getAttribute("style:break-inside", props);
    if (val)
      m_keepTogether = UT_String_sprintf ("keep-together: %s;", !UT_strcmp(val, "avoid") ? "yes" : "no" );
...
public:
  ...
  void defineSimpleStyle (const UT_UTF8String & name, const XML_Char **props);
  const XML_Char* mapStyle (const XML_Char * name) const;
  const OO_Style * mapStyleObj (const XML_Char * name) const;
...
bool IE_Imp_OpenWriter_Sniffer::getDlgLabels (const char ** szDesc,
					      ...
					      IEFileType * ft)
{
  *szDesc = "OpenOffice Writer (.sxw, .odt)";
  *szSuffixList = "*.sxw; *.odt";
  *ft = getFileType();
  return true;
}

void IE_Imp_OpenWriter::defineSimpleStyle (const UT_UTF8String & name, const XML_Char **props)
{
  if (!name.size() || !props)
    return;
...
const XML_Char* IE_Imp_OpenWriter::mapStyle (const XML_Char * name) const
{
...
const OO_Style * IE_Imp_OpenWriter::mapStyleObj (const XML_Char * name) const
{
  if (!name)
    return NULL;
...
IE_Imp_OpenWriter::IE_Imp_OpenWriter (PD_Document * pDocument)
  : IE_Imp (pDocument), m_oo (0), m_pSSListener(0), m_bOpenDocument(false)
{
}
...
UT_Error IE_Imp_OpenWriter::importFile(const char * szFilename)
{
  ...
  if ( UT_OK != _handleMimetype ())
    ; // TODO: give warning about missing mimetype stream
...
UT_Error IE_Imp_OpenWriter::_handleMimetype ()
{
  ...
  m_bOpenDocument = FALSE;

  if (!input) {
    UT_DEBUGMSG(("Error: didn't get a mimetype. Assuming that it's a application/vnd.sun.xml.writer document\n"));
    UT_ASSERT(UT_SHOULD_NOT_HAPPEN);
    return UT_OK;
  }
  ...
  if (!UT_strcmp("application/vnd.oasis.opendocument.text", mimetype.utf8_str())) {
    m_bOpenDocument = TRUE;
  }
  else if (!UT_strcmp("application/vnd.sun.xml.writer", mimetype.utf8_str())) {
  }
  else {
...
public:
  ...
  {
	/* SXW || ODT */
    if (!UT_strcmp (name, "style:page-master") || !UT_strcmp (name, "style:page-layout")) {
      m_pageMaster = UT_getAttribute("style:name", atts);
    }
    else if (!UT_strcmp (name, "style:master-page")) {
      const XML_Char * masterName = UT_getAttribute("style:page-master-name", atts);
  ...
  virtual void endElement (const XML_Char * name)
  {
    if (!UT_strcmp (name, "style:page-master")) {
      m_pageMaster = NULL;
    }
    else if (!strcmp (name, "style:style")) {
      if (m_name.size ()) {
...
public:
  ...
  virtual void startElement (const XML_Char * name, const XML_Char ** atts) 
  {
    ...
    else if (!UT_strcmp(name, "text:section" ))
      {	
    ...
    else if (!UT_strcmp(name, "text:p" ) || !UT_strcmp(name, "text:h" ))
      {
    ...
    else if (!UT_strcmp(name, "text:ordered-list") || !UT_strcmp(name, "text:unordered-list"))
      {
	...
	if (!UT_strcmp(name, "text:ordered-list"))
	...
	m_curStyleName = UT_getAttribute ("style:name", atts);
			...
			!UT_strcmp(name, "style:properties") || 
			!UT_strcmp(name, "style:columns") ||
			/* ODT */
			!UT_strcmp(name, "style:text-properties"))
      {
    ...
    else if (!UT_strcmp(name, "text:table-of-content"))
    {
      _flush ();
      _insureInBlock(NULL);
  ...
  virtual void endElement (const XML_Char * name)
  {
    if (!UT_strcmp(name, "text:section" ))
      {
	m_bInSection = false;
      }
    else if (!UT_strcmp(name, "text:p" ) || !UT_strcmp(name, "text:h" ))
...
private:
  ...
  void _insureInBlock(const XML_Char ** atts)
  {
    ...
    if (!m_bAcceptingText) {
      getDocument()->appendStrux(PTX_Block, (const XML_Char**)atts);	
      m_bAcceptingText = true;
    }
  }

  void _insureInSection(const XML_Char * props) 
  {
    if (m_bInSection)
      return;
    ...
    atts[2] = 0;
    getDocument()->appendStrux(PTX_Section, (const XML_Char**)atts);	

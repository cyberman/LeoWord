...
UT_Confidence_t IE_Imp_KWord_1_Sniffer::supportsMIME (const char * szMIME)
{
  if (UT_strcmp (IE_FileInfo::mapAlias (szMIME), IE_MIME_KWord) == 0)
    {
...
void IE_Imp_KWord_1::startElement(const XML_Char *name, const XML_Char **atts)
{
  ...
  switch (tokenIndex)
  {
	...
	propsArray[1] = static_cast<const XML_Char *>(m_szSectProps.c_str());
	propsArray[2] = 0;

	X_CheckError(appendStrux(PTX_Section,static_cast<const XML_Char**>(&propsArray[0])));
    ...
    case TT_ITALIC:
      {
        pVal = static_cast<const XML_Char *>(_getXMLPropValue("value", atts));
    ...
    case TT_FONT:
      {
        pVal = static_cast<const XML_Char *>(_getXMLPropValue("name", atts));
        if (pVal)
        {
...
void IE_Imp_KWord_1::endElement(const XML_Char *name)
{
  ...
  switch (tokenIndex)
  {
    ...
    case TT_FORMAT:
    {
      ...
      propsArray[0] = static_cast<const XML_Char *>("props");
      propsArray[1] = static_cast<const XML_Char *>(m_szCharProps.c_str());
      propsArray[2] = 0;

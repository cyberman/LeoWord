    ...
    PT_DocPosition endpos )
{
    ...
    for( PT_DocPosition pos = currentpos; pos < endpos; )
    {
        ...
        {
            auto pft = static_cast<const pf_Frag_Text*>(pf);
        ...
        {
			auto po = static_cast<const pf_Frag_Object*>(pf);
            std::string typeStr = "";
        ...
        {
            PTStruxType eStruxType = static_cast<const pf_Frag_Strux*>(pf)->getStruxType();
...
pf_Frag* pt_PieceTable::getEndOfBlock( PT_DocPosition currentpos, PT_DocPosition endpos )
{
    ...
    if (getMutFragFromPosition(currentpos, &pf, &Offset))
    {
        if( tryDownCastStrux( pf, PTX_Block ))
        {
            ++currentpos;
    ...
    {
        if (!getMutFragFromPosition(pos, &pf, &Offset))
        {
...
bool pt_PieceTable::_tweakDeleteSpanOnce(PT_DocPosition & dpos1,
										 PT_DocPosition & dpos2,
										 StruxStack& pstDelayStruxDelete) const
{
			...
			if (pt_BeforeType == PTX_Block && pt_LastType == PTX_Block)
			{
				...
				//  to deleting the block later.
					...
					if ((pfs_BeforeSection == pfs_LastSection) && (dpos2 > dpos1 +1))
					{
						...
						pstDelayStruxDelete.push(static_cast<pf_Frag_Strux*>(pf_Before));
						return true;
					}
				}
			}
...
//
	...
	while (1)
	{
		...
		std::size_t old_iStackSize = pstDelayStruxDelete.size();

		if(!_tweakDeleteSpanOnce(dpos1, dpos2, pstDelayStruxDelete))
			return false;

		if (dpos1 == old_dpos1 && dpos2 == old_dpos2
			&& pstDelayStruxDelete.size() == old_iStackSize)
			return true;
	}
}
 ...
 */
bool pt_PieceTable::_StruxIsNotTable(const pf_Frag_Strux * pfs) const
{
									...
									bool bDontGlob)
{
	...
	bool bSuccess = true;
	pt_PieceTable::StruxStack stDelayStruxDelete;
	...
	bool bIsSimple = _isSimpleDeleteSpan(dpos1, dpos2) && stDelayStruxDelete.size() == 0;
	if (bIsSimple)
	{
	...
	else
	{
		...
		std::size_t oldDepth = stDelayStruxDelete.size();
		...
		while (bSuccess && stDelayStruxDelete.size() > 0)
		{
			pf_Frag_Strux * pfs;
			if(stDelayStruxDelete.size() <= oldDepth)
			{
...
//													  &pf, &dp);
		...
		(p_frag_after->getType() == pf_Frag::PFT_EndOfDoc)))
	{
		...
		{
		     auto pfsa = static_cast<const pf_Frag_Strux *>(p_frag_after);
		     if(isEndFootnote(pfsa))
		     {
			 	bDoit = false;
		     }
		}
		auto pfs = static_cast<const pf_Frag_Strux *>(p_frag_before);
...
bool pt_PieceTable::deleteFieldFrag(const pf_Frag * pf)
{
	UT_return_val_if_fail (m_pts==PTS_Editing,false);
                                    ...
                                    PT_DocPosition & dpos2) const
{
	...
	const pf_Frag * pf_First;
	const pf_Frag * pf_End;
	const pf_Frag * pf_Other;
	PT_BlockOffset fragOffset_First;
	PT_BlockOffset fragOffset_End;
    ...
    {
        const fd_Field * pField = pf_End->getField();

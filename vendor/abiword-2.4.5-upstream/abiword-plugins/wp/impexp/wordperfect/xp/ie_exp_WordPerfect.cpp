...
bool IE_Exp_WordPerfect_Sniffer::recognizeSuffix (const char * szSuffix)
{
  ...
  return ( !UT_stricmp(szSuffix,".wpd") || !UT_stricmp(szSuffix, ".wp") ) ;
}
...
bool IE_Exp_WordPerfect::_openFile(const char * szFilename)
{
	UT_DEBUGMSG(("WordPerfect Exporter: _openFile: %s\n", szFilename));	
	return IE_Exp::_openFile(szFilename);
}

bool IE_Exp_WordPerfect::_closeFile()
{
	UT_DEBUGMSG(("WordPerfect Exporter: _closeFile\n"));	

 ...
 * 
 */
void ODi_StreamListener::_startElement (const XML_Char* pName,
 ...
 * 
 */
void ODi_StreamListener::_endElement (const XML_Char* pName, bool doingRecursion)
{
 ...
 * 
 */
void ODi_StreamListener::charData (const XML_Char* pBuffer, int length)
{
    ...
    switch (m_stateAction.getAction()) {
        ...
        case ODi_ListenerStateAction::ACTION_PUSH:
            ...
            if (m_stateAction.getState() != NULL) {
                ...
                if (!UT_strcmp(m_stateAction.getStateName().c_str(),
                               "FontFaceDecls")) {

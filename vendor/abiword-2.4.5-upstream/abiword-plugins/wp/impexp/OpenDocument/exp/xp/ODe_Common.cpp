 ...
 * 
 */
void ODe_writeToFile(FILE* pFile, const UT_UTF8String& rString) {
    size_t result;

    result = fwrite(rString.utf8_str(), 1, rString.byteLength(), pFile);
    UT_ASSERT(result == rString.byteLength());
}
 ...
 */
void ODe_writeToFile(FILE* pDestinationFile, FILE* pSourceFile) {
    size_t bytesRead;
    size_t bytesWritten;
    ...
    while (feof(pSourceFile) == 0) {
        bytesRead = fread((void *)byteBuffer, 1, 1000, pSourceFile);
        
        bytesWritten = fwrite((void *)byteBuffer, 1, bytesRead, pDestinationFile);
        
        UT_ASSERT(bytesRead == bytesWritten);
    }
}

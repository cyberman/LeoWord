...
bool ODe_ManifestWriter::writeManifest(PD_Document* pDoc, GsfOutfile* pODT)
{
    ...
    ODe_writeToStream (manifest, preamble, NrElements(preamble));
    ...
    for (UT_uint32 k = 0;
        ...
        if (szMimeType && !UT_strcmp(szMimeType, "image/png")) {
    ...
    ODe_writeToStream (manifest, postamble, NrElements(postamble));
